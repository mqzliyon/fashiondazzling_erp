...
export default function ForeignShipmentPage() {
  useAuthGuard({ requireAuth: true });
  const [items, setItems] = useState<ForeignShipmentItem[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [activeItem, setActiveItem] = useState<ForeignShipmentItem | null>(null);
  ...
  const loadShipments = async () => {
    ...
    setError(null);
    try {
      const res = await api.get<{ data: ForeignShipmentItem[] }>("/foreign-shipments");
      setItems(res.data.data || []);
    } catch {
      setError("Failed to load foreign shipment list.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const openDetails = (item: ForeignShipmentItem) => {
    setActiveItem(item);
    setShowDetailsModal(true);
  };

  const deleteShipment = async (item: ForeignShipmentItem) => {
    setDeletingId(item._id);
    setError(null);
    try {
      await api.delete(`/foreign-shipments/${item._id}`);
      setShowDetailsModal(false);
      await loadShipments();
    } catch {
      setError("Failed to delete foreign shipment.");
    } finally {
      setDeletingId(null);
    }
  };
  ...
  const filteredItems = items.filter((item) => {
    const matchesLot = lotSearch
      ...
      : true;
    const dateTs = new Date(item.shipmentDate).getTime();
  ...
  return (
    <main className="min-h-screen bg-slate-50 p-6 md:p-8">
      <section className="mx-auto max-w-6xl space-y-5">
        <header>
          <h1 className="text-2xl font-semibold">Foreign Shipment</h1>
          <p className="text-sm text-muted-foreground">
            Export shipment list with buyer details and transfer source.
          </p>
        </header>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b text-left">
                      ...
                      <th className="py-2 pr-4">Buyer Phone</th>
                  ...
                  <tbody>
                    {filteredItems.map((item) => (
                      ...
                      >
                        ...
                        <td className="py-2 pr-4">{item.buyerName || "-"}</td>
                        <td className="py-2 pr-4">{item.country}</td>
                        <td className="py-2 pr-4">{item.buyerPhone || "-"}</td>
                        <td className="py-2 pr-4">{item.quantity} pcs</td>
                        <td className="py-2 pr-4">{new Date(item.shipmentDate).toLocaleDateString()}</td>
                    ...
                    {filteredItems.length === 0 && (
                      <tr>
                        <td colSpan={7} className="py-8 text-center text-muted-foreground">
                          No foreign shipment entries found.
                        </td>
                      </tr>
                    )}
                  </tbody>
      ...
      <Dialog open={showDetailsModal} onOpenChange={setShowDetailsModal}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Foreign Shipment Details</DialogTitle>
          </DialogHeader>
          {activeItem && (
            <div className="grid gap-2 text-sm">
              <p><span className="font-medium">Lot Number:</span> {activeItem.lot?.lotNumber || "N/A"}</p>
              <p><span className="font-medium">Buyer Name:</span> {activeItem.buyerName || "-"}</p>
              <p><span className="font-medium">Buyer Country:</span> {activeItem.country}</p>
              <p><span className="font-medium">Buyer Phone:</span> {activeItem.buyerPhone || "-"}</p>
              ...
              <p><span className="font-medium">Shipment Number:</span> {activeItem.shipmentNumber}</p>
              <p><span className="font-medium">Status:</span> {activeItem.status}</p>
              <p><span className="font-medium">Date:</span> {new Date(activeItem.shipmentDate).toLocaleString()}</p>
              <div className="mt-2 flex justify-end gap-2">
                ...
                <Button
                  ...
                  onClick={() => deleteShipment(activeItem)}
